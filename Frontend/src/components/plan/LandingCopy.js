...
`;
const LandingCopy = ({ onHandlePageOut }) => {
	...
	const userLocation = () => {
		...
		if (navigator.geolocation) {
			...
			navigator.geolocation.getCurrentPosition(
				(position) => {
					searchDetailAddFromCoords(position.coords.latitude, position.coords.longitude, function (result, status) {
						if (status === kakao.maps.services.Status.OK) {
					...
					bounds.extend(new kakao.maps.LatLng(position.coords.latitude, position.coords.longitude));
					setMap((prev) => ({
						...prev,
						center: {
			...
			<MapWrapper>
				<Map
					...
					onClick={(_t, mouseEvent) => {
						...
						if (!map) return;
						const bounds = new kakao.maps.LatLngBounds();
						searchDetailAddFromCoords(mouseEvent.latLng.getLat(), mouseEvent.latLng.getLng(), function (result, status) {
							if (status === kakao.maps.services.Status.OK) {
								console.log(result[0]);
								roadAddr = !!result[0].road_address ? result[0].road_address.address_name : "";
								jibunAddr = result[0].address.address_name;
						...
						//   },
						// });
						bounds.extend(new kakao.maps.LatLng(mouseEvent.latLng.getLat(), mouseEvent.latLng.getLng()));
						map.setBounds(bounds);
						map.setLevel(4);
						map.setCenter(new kakao.maps.LatLng(mouseEvent.latLng.getLat(), mouseEvent.latLng.getLng()));
						// setMarkers(markers);
					}}
					// ref={mapRef}
				>
