...
const PlanMakeCard = () => {
  ...
  const [reloadAudio] = useState(
    new Audio(`${process.env.PUBLIC_URL}/assets/sounds/reload.mp3`)
  );
  ...
  const onHandleList = (listType, index) => {
    ...
    if (listType === ListTypes.RECOMMEND) {
      // [추천] -> [약속]
      const arr1Index =
        (page * ELEMENTS_PER_PAGE + index) % recommendList.length;
  ...
  return (
    <div align="center">
      ...
      {modalToggle ? (
        <CardDetail modalClose={openModal} placeId={modalPlaceId} />
      ) : null}
          ...
          <PageHeaderBlock height="calc(20vh)">
            ...
            <TrashCan onClick={() => setTrashToggle(!trashToggle)}>
              <img
                ...
                src={
                  !trashToggle
                    ? `${process.env.PUBLIC_URL}/assets/plan/trash_close.png`
                    : `${process.env.PUBLIC_URL}/assets/plan/trash_open.png`
                }
                alt="휴지통"
                width="100%"
              />
            ...
            <TrashWrapper
              height={trashToggle ? "600px" : "0px"}
              mHeight={trashToggle ? "50%" : "0px"}
            >
              <PlanList
                pList={trashList}
                setPList={setTrashList}
                openModal={openModal}
                listType={ListTypes.TRASH}
          ...
          <PlanBlock justifyContent="space-between">
            <PlanMakeWrapper width="calc(50% - 1rem)">
              ...
              <RadiusWrapper>
                ...
                <RadiusButton
                  onClick={() => onHandleRadius(recommendListToggle)}
                >
                  설정
                </RadiusButton>
              </RadiusWrapper>
            ...
            <PlanMakeWrapper width="calc(25% - 1rem)">
              <BaseFlexWrapper>
                <SectionTitle
                  width="50%"
                  bg={recommendListToggle ? "#80E080" : "#D9D9D9"}
                  onClick={() => onHandleChangeList(true)}
                >
                  추천
                </SectionTitle>
                <SectionTitle
                  width="50%"
                  bg={!recommendListToggle ? "#80E080" : "#D9D9D9"}
                  onClick={() => onHandleChangeList(false)}
                >
                  목록
                </SectionTitle>
              </BaseFlexWrapper>
