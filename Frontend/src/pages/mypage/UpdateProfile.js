...
import { login, nickNameDuplicateCheck } from "../../api/AuthAPI";
...
const UpdateProfile = () => {
	...
	//   ACCtoken = response.token.access;
	...
	const onHandleNickNameDuplicateCheck = (e) => {
		...
		if (response.code === 200) {
			const finish = confirm("사용 가능한 닉네임입니다. 이 닉네임으로 변경하시겠습니까?");
			if (finish) {
				setProfile({ ...profile, ["userNickNameChecked"]: true });
			}
	...
	return (
		<>
			<Modal
				isOpen={modalState}
				modalContent={<ProfileImageListContent profileImageState={profileImgState} onClcik={onHandleChangeProfileImage} close={closeModal} />}
			/>
			<ProfileTitle isEdit={true} openModal={openModal} imageId={profileImgState} userName={profile.userName} userEmail={profile.userEmail} />
			<SignupBody>
				...
				<InputBlock>
					<InputCheckButton message="되돌리기" width="40%" clickEvent={() => returnState()}></InputCheckButton>
					<InputCheckButton message="저장" width="40%" borderColor="#80E080" color="#80C0A0" clickEvent={() => submitState()}></InputCheckButton>
				</InputBlock>
			</SignupBody>
		</>
	);
