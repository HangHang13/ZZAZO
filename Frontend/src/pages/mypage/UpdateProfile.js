...
import { InputFullWrapper, InputWrapper } from "../../components/styled/Wrapper";
...
const UpdateProfile = () => {
	...
	const onHandleNickNameDuplicateCheck = async (e) => {
		...
		if (user.data.userNickName === profile.userNickName || result.code === 200) {
	...
	return (
		<>
			<Modal
				isOpen={modalState}
				modalContent={<ProfileImageListContent profileImageState={profileImgState.imgId} onClick={onHandleChangeProfileImage} close={closeModal} />}
			/>
			<ProfileTitle isEdit={true} openModal={openModal} imageId={profileImgState.imgId} userName={profile.userName} userEmail={profile.userEmail} />
			<SignupBody>
				...
				<InputBlock>
					<Button message="되돌리기" width="40%" clickEvent={() => returnState()}></Button>
					<Button message="저장" width="40%" borderColor="#80E080" color="#80C0A0" clickEvent={() => submitState()}></Button>
				</InputBlock>
			</SignupBody>
		</>
	);
